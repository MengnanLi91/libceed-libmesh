...
void
assemble_poisson_system(EquationSystems & es, const std::string & system_name)
{
...
AssemblySystem::AssemblySystem()
  : _fe(2, libMesh::FEType(libMesh::SECOND, libMesh::LAGRANGE), libMesh::SECOND)
{
}

void
AssemblySystem::assembleCEED(FEproblemData & feproblem_data, CeedSetup & ceedsetup)
{
  ...
  // volume example
  // ceedsetup.buildCartesianRestriction(feproblem_data, feproblem_data.num_comp,
  // &feproblem_data.mesh_size, &ceedsetup.elem_restr_x, NULL);
  // ceedsetup.buildCartesianRestriction(feproblem_data, 1, &feproblem_data.sol_size,
  // &ceedsetup.elem_restr_u, &ceedsetup.elem_restr_qd);

  ceedsetup.buildCartesianRestriction(feproblem_data,
                                      feproblem_data.num_comp,
                                      &feproblem_data.mesh_size,
                                      &ceedsetup.elem_restr_x,
                                      NULL);
  ceedsetup.buildCartesianRestriction(feproblem_data,
                                      dim * (dim + 1) / 2,
                                      &feproblem_data.sol_size,
                                      NULL,
                                      &ceedsetup.elem_restr_qd);
  ceedsetup.buildCartesianRestriction(
      feproblem_data, 1, &feproblem_data.sol_size, &ceedsetup.elem_restr_u, NULL);
...
Real
AssemblySystem::exact_solution(const Real x, const Real y, const Real z)
{
...
void
AssemblySystem::assembleFunc(libMesh::EquationSystems & equation_systems,
