  ...
  switch (build_data->dim + 10 * build_data->space_dim)
  {
    ...
    case 33:
      CeedPragmaSIMD for (CeedInt i = 0; i < Q; i++)
      {
        ...
        const CeedScalar qw =
            w[i] / (J[i + Q * 0] * A[0][0] + J[i + Q * 1] * A[0][1] + J[i + Q * 2] * A[0][2]);
  ...
  switch (build_data->dim)
  {
    ...
    case 2:
      CeedPragmaSIMD for (CeedInt i = 0; i < Q; i++)
      {
        ...
        // 0 2
        // 2 1
        const CeedScalar dXdxdXdx_T[2][2] = {{q_data[i + 0 * Q], q_data[i + 2 * Q]},
    ...
    case 3:
      CeedPragmaSIMD for (CeedInt i = 0; i < Q; i++)
      {
        ...
        for (int j = 0; j < 3; j++)
          vg[i + j * Q] =
              (du[0] * dXdxdXdx_T[0][j] + du[1] * dXdxdXdx_T[1][j] + du[2] * dXdxdXdx_T[2][j]);
      } // End of Quadrature Point Loop
      break;
  }
  return 0;
